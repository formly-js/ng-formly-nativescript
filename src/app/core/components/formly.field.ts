...
export class FormlyField implements DoCheck, OnInit {
  ...
  private createFieldComponents() {
    if (this.field && !this.field.template && !this.field.fieldGroup && !this.field.fieldArray) {
      ...
      if (this.field.key) {
        let valueChanges = (<any>fieldComponentRef.instance).valueChanges;
        if (debounce > 0) {
          valueChanges = valueChanges.debounceTime(debounce);
        }
        valueChanges.subscribe((value) => {
          console.log(value);
          this.changeModel(new FormlyValueChangeEvent(this.field.key, value));
        });
        // let valueChanges = fieldComponentRef.instance.formControl.valueChanges;
        // console.log('got valueChange in FormlyField:', valueChanges);
        // if (debounce > 0) {
        //   valueChanges = valueChanges.debounceTime(debounce);
        // }
        ...
        // valueChanges.subscribe((event) => this.changeModel(new FormlyValueChangeEvent(this.field.key, event)););
      }
  ...
  private createComponent(fieldComponent, component): ComponentRef<any> {
    ...
    (<any>Object).assign(ref.instance, {
        model: this.model,
        form: this.form,
        field: this.field,
